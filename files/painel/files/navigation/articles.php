<?php 
	...
	if (isset($username)) {
		...
		if ($consult_articles_by_name->rowCount() > 0) {
			...
			$consult_my_articles = $db->prepare("SELECT * FROM cms_news WHERE author = ?");
	...
	if ($username == $result_panel_user['username']) {
		...
		$page_title = 'Painel: Minhas noticias - ' . HOTELNAME;
	} else {
		$page_name = 'User-Articles';
		$page_title = 'Painel | Noticias: ' . $username . ' - ' . HOTELNAME;
	}
			...
			<div class="content flex-column">
				<div class="header-content flex">
					...
					<label class="mr-auto-top-bottom">
						<h2 class="bold">Minhas noticias</h2>
					</label>
				</div>
				<div class="content-container">
					...
					<?php
						if ($consult_my_articles->rowCount() > 0) {
							while ($result_my_articles = $consult_my_articles->fetch(PDO::FETCH_ASSOC)) {
								...
								$consult_article_forms = $db->prepare("SELECT * FROM cms_post_forms WHERE post_id = ? AND type = ?");
								$consult_article_forms->bindValue(1, $result_my_articles['id']);
								$consult_article_forms->bindValue(2, 'article');
					...
					?>
						<div class="my-article-box card" data-article-id="<?= $result_my_articles['id']; ?>">
							...
							<div class="my-article-box-label">
								<label class="flex-column color-5">
									...
									<div class="flex-column">
										...
										<?php if ($result_my_articles['form'] == '1') { ?>
										<h5><span class="bold"><?= $consult_article_forms->rowCount(); ?></span> formulário<?php if ($consult_article_forms->rowCount() > 1 || $consult_article_forms->rowCount() == 0) { ?>s<?php } ?></h5>
										<?php } ?>
									</div>
								</label>
							</div>
							...
							<?php if ($result_my_articles['form'] == '1') { ?>
								<a href="<?= URL_PANEL . '/article/' . $result_my_articles['id'] . '/forms'; ?>" class="my-article-action-forms no-link">Ver formulários</a>
							<?php } ?>
								<div class="mr-auto-left flex">
									<a href="<?= URL_PANEL . '/article/' . $result_my_articles['id'] . '/edit'; ?>" class="my-article-action-shelve mr-right-2 no-link">Editar</a>
